...
/*   Updated: 2022/11/24 11:40:13 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*used when an entire buffer doesn't contained any \n : concatenate all the
 buffer to the given string rtn (malloc & free) and refill the buffer with a
 new read*/
int	ft_newbuff(t_buffer *b, char **rtn, int fd)
{
	char	*tmp;

	tmp = ft_join(*rtn, b->t, b->s, b->e - b->s);
	free(*rtn);
	*rtn = tmp;
	b->e = read(fd, b->t, BUFFER_SIZE);
	if (b->e == -1 || !*rtn)
